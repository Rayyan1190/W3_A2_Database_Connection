...
The first time it starts, it automatically creates a `tasks.db` file next to `main.py`, creates the `tasks` table if it does not already exist, and seeds it with three example tasks if the table is empty. On every startup after that, the seeding step is skipped since the table already has data in it.
...
CRUD-APi/
    ...
    tasks.db            the SQLite database file, created automatically on first run
